...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(
        ...
        Step size is givenas a fraction of the patch_size. 0.5 is the default and means that wen advance by
        patch_size * 0.5 between predictions. step_size cannot be larger than 1!
        ...
        if len(mirror_axes):
            ...
            if self.conv_op == nn.Conv3d or self.conv_op == cotr.Conv3d_wd:
    ...
    def predict_2D(
        ...
        :param step_size: When running sliding window prediction, the step size determines the distance between adjacent
        predictions. The smaller the step size, the denser the predictions (and the longer it takes!).
        Step size is given as a fraction of the patch_size. 0.5 is the default and means that wen advance
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    # Myr : We add the shape in the volume size list and the crop center position in the
                    # corresponding list.
                    shape = data.shape[1:]
                    # volume_size[j] = shape
