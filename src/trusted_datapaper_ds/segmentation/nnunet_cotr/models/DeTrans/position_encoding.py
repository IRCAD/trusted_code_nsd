...
class PositionEmbeddingSine(nn.Module):
    ...
    def __init__(
        self, num_pos_feats=[64, 64, 64], temperature=10000, normalize=False, scale=None
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        dim_tx = torch.arange(
            self.num_pos_feats[0], dtype=torch.float32, device=x.device
        )
        dim_tx = self.temperature ** (3 * (dim_tx // 3) / self.num_pos_feats[0])

        dim_ty = torch.arange(
            self.num_pos_feats[1], dtype=torch.float32, device=x.device
        )
...
def build_position_encoding(mode, hidden_dim):
    ...
    if mode in ("v2", "sine"):
        position_embedding = PositionEmbeddingSine(
            num_pos_feats=N_steps, normalize=True
        )
    else:
