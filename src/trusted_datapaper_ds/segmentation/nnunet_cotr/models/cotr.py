...
class U_ResTran3D(nn.Module):
    def __init__(
        ...
        self.backbone = CNNBackbone.Backbone(
            depth=9, norm_cfg=norm_cfg, weight_std=weight_std
        )
    ...
    def forward(self, inputs, *args, **kwargs):
        ...
        agno = (
            x_trans.shape[1]
            - x_convs[-1].shape[-1] * x_convs[-1].shape[-2] * x_convs[-1].shape[-3]
        )

        x = self.transposeconv_stage2(
            x_trans[:, agno::].transpose(-1, -2).view(x_convs[-1].shape)
