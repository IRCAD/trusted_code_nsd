...
def upsampling_indiv(refpath, inpath, output_folder, clean=True):
    ...
    for ref, ind in zip(refpath_list, inpath_list):
        imgpath = ref
        maskpath = ind
...
def upsampling_list(refpath_list, inpath_list, output_folder, clean=True):
    makedir(output_folder)
...
def main(refpath_list, inpath_list, output_folder):
    upsampling_list(refpath_list, inpath_list, output_folder, clean=True)
...
if __name__ == "__main__":
    ...
    refpath_list = natsorted(glob(join(ref_folder, "*.nii.gz")))
    inpath_list = natsorted(glob(join(input_folder, "*.nii.gz")))

    output_folder = join(
        config["seglocation"], config["segmodel"], config["training_target"]
    )

    main(config, refpath_list, inpath_list, output_folder)
