...
# and `pip install trusted_datapaper_ds` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "trusted_datapaper_ds", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n trusted_datapaper_ds -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "trusted_datapaper_ds", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "trusted_datapaper_ds", "YOUR_CONSOLE_SCRIPT"]
